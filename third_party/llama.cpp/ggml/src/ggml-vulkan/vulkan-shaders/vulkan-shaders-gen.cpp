...
compile_count_guard acquire_compile_slot() {
    ...
    // RESEARCH(vendor fix): on Windows the concurrent glslc jobs race on shared
    // output paths ("cannot open output file"), and a retried ninja build only
    // converges slowly, which can silently keep STALE .comp.cpp files.
    // GGML_VK_SHADER_GEN_THREADS=1 makes generation deterministic.
    uint32_t N = std::max(1u, std::min(16u, std::thread::hardware_concurrency()));
    if (const char * t_ = getenv("GGML_VK_SHADER_GEN_THREADS")) {
        N = std::max(1u, (uint32_t) atoi(t_));
    }
